...
test('package metadata and license declare free open-source distribution', () => {
  ...
  const metadataGenerator = fs.readFileSync('scripts/release-metadata.cjs', 'utf8');
  ...
  assert.doesNotMatch(metadataGenerator, /Aux Command itself remains[\s\S]{0,40}proprietary/u);
  assert.match(metadataGenerator, /AGPL-3\.0-or-later/u);
});
